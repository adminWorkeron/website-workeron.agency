...
Flask server with SQLite persistence
"""
...
CORS(app, origins=['https://workeron.agency', 'http://localhost:*'], 
...
def init_db():
    ...
    if openrouter_key:
        ...
        if not existing_key or not existing_key['value']:
            db.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('apiKey', ?)", (openrouter_key,))
            db.commit()
            print(f"API key configured from environment")
...
from api_external import register_external_api
register_external_api(app, get_db)
@app.after_request
